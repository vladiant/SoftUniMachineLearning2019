...
int main(int argc, char* argv[]) {
  ...
  logistic_regression->train(raw_data->getTrainSamples(), 0,
  ...
  for (int i = 0; i < raw_data->getTrainSamples().rows; i++) {
    const auto predicted =
        logistic_regression->predict(raw_data->getTrainSamples().row(i));
  ...
  std::cout << "Missclassified test samples, %: "
            << logistic_regression->calcError(raw_data, true, cv::noArray())
            << std::endl;

  for (int i = 0; i < raw_data->getTestSamples().rows; i++) {
    const auto predicted =
        logistic_regression->predict(raw_data->getTestSamples().row(i));
