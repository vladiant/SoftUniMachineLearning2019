...
int main(int argc, char* argv[]) {
  ...
  auto d_tree = cv::ml::DTrees::create();
  ...
  // This causes crash!
  d_tree->setMaxDepth(40);

  // Note: Bigger tree depth causes overfitting!

  // Check documentation
  //  d_tree->setMaxCategories(10);
  //  d_tree->setMinSampleCount(10);

  //  TODO: Ivestigate whether prior has to be applied for
  //  non-uniform distribution of labels to enforce stratification
  //  d_tree->setPriors(cv::Mat());

  //  d_tree->setRegressionAccuracy(0.01f);
  //  d_tree->setTruncatePrunedTree(true);
  //  d_tree->setUse1SERule(true);
  //  Value 'true' not implemented!
  //  d_tree->setUseSurrogates(false);

  std::cout << "Start training" << std::endl;

  // Train with the whole dataset
  d_tree->train(raw_data->getTrainSamples(), 0, raw_data->getTrainResponses());
  ...
  std::cout << "Missclassified train samples, %: "
            << d_tree->calcError(raw_data, false, cv::noArray()) << std::endl;
